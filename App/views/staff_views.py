...
@jwt_required()
def api_list_staff():
    # Only admins may list all staff
...
def api_get_roster():
    """
    GET /api/v1/roster?week_start=YYYY-MM-DD (optional)
    Returns roster details for the requested week or current week
    """
    ...
    result = []
    for sh in shifts:
        staff = db.session.get(type(sh).metadata.tables.get("staff") and None)  # keep previous behaviour
        # safer: manually get staff object by ID
        from App.models.staff import Staff as StaffModel
        staff_obj = db.session.get(StaffModel, sh.staffId)
    ...
    return jsonify({"weekStart": roster.weekStartDate.isoformat(), "weekEnd": roster.weekEndDate.isoformat(), "shifts": result}), 200

@staff_bp.route("/shifts/<int:shift_id>/time-in", methods=["POST"])
@jwt_required()
def api_time_in(shift_id):
    """
    Staff records time in for a shift. Body optional: {"timestamp":"YYYY-MM-DDTHH:MM"}
    """
    # Ensure staff only marks their own shift
    ...
    if ts:
        try:
            ...
            return jsonify({"msg": "invalid timestamp"}), 400
    else:
        ts_dt = None
...
def api_time_out(shift_id):
    """
    Staff records time out for a shift. Body optional: {"timestamp":"YYYY-MM-DDTHH:MM"}
    """
    uid = current_user.userId
    from App.models.shift import Shift
    sh = db.session.get(Shift, shift_id)
...
def api_generate_report(roster_id):
    # Only admin can explicitly create/generate reports
    if not is_admin(current_user):
        return jsonify({"msg": "admin only"}), 403
